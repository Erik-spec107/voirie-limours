...
class MainActivity : AppCompatActivity() {
    ...
    private fun launchCamera() {
        val timeStamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.FR).format(Date())
    ...
    private fun saveDesordre() {
        ...
        val now = Date()
        val dateFmt = SimpleDateFormat("dd/MM/yyyy", Locale.FR)
        val heureFmt = SimpleDateFormat("HH:mm", Locale.FR)
    ...
    private fun renderListe() {
        val db = getDB()
        val today = SimpleDateFormat("dd/MM/yyyy", Locale.FR).format(Date())
